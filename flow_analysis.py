...
print(
    f"Getting resolution from file {resolution_file_name}, histogram {res_histo_name}"
)
...
for i_cent in range(n_cent_classes):
    ...
    flow_maker.harmonic = harmonic

    if useSP:
        flow_maker.v2_bin_limits = [-10.0, 10.0]
        flow_maker.n_v2_bins = 100
        flow_maker.v2_axis_label = r"Q cos(%d(#phi - #Psi_{%d}))" % (harmonic, harmonic)
    else:
        flow_maker.v2_axis_label = r"cos(%d(#phi - #Psi_{%d}))" % (harmonic, harmonic)
