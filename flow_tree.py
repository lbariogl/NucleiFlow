...
if do_syst:
    ...
    if n_trials < len(mega_combos):
        mega_combo_random_indices = np.random.randint(
            len(mega_combos), size=n_trials)
    else:
        print(
    ...
    for i_cent in range(n_cent_classes):
        ...
        for i_mega_combo, mega_combo_index in enumerate(mega_combo_random_indices):
            ...
            print(
                f'pt-dependent selections: {flow_maker_syst.ptdep_selection_string}')
        ...
        for i_pt in range(0, n_pt_bins):
            ...
            std_line = ROOT.TLine(
                default_values[i_cent][i_pt][0], 0, default_values[i_cent][i_pt][0], 1.05 * histo_v2_syst[i_pt].GetMaximum())
