...
hPhiResolution = ROOT.TH2D(
    ...
    ";#it{p}_{T} (GeV/#it{c});%s;" % (z_axis.GetTitle()),
    ny,
    y_axis.GetXmin(),
    y_axis.GetXmax(),
    nz,
...
hPhiResolution.SetTitle(r";#it{p}_{T} (GeV/#it{c}); #phi^{rec} - #phi^{MC} (rad);")
hPhiResolution.GetYaxis().SetRangeUser(-2.0, 2.0)
hPhiResolution.GetXaxis().SetRangeUser(0.49, 5.0)
...
hRMSVsPt.GetXaxis().SetRangeUser(0.49, 5.0)
...
canvas = ROOT.TCanvas("cPhiResolution2D", "Phi Resolution 2D and Profile", 900, 700)
...
canvas.SaveAs("../results_phi_studies/cPhiResolution2D.pdf")
canvas.Write()
...
canvas_fit = ROOT.TCanvas("cPhiResolution", "RMS vs pT with Fit", 800, 600)
canvas_fit.SetRightMargin(0.15)  # Increase right margin for profile
hRMSVsPt.Draw("PE")
fit_func.SetLineColor(ROOT.kRed)
fit_func.Draw("L SAME")
canvas_fit.SaveAs("../results_phi_studies/cPhiResolution.pdf")
canvas_fit.Write()
