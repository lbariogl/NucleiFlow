...
for det in detectors:
    ...
    for i_cent, cent in enumerate(centrality_classes):
        ...
        det_dir.cd()
        hPsi.Write()
    formatted_title = f";#Psi_{{{harmonic}}}^{det}; norm. counts"
...
vCos4PhiMinusPsiVsPt = {}
cSinPhiMinusPsiVsPt = {}
cCos2PhiMinusPsiVsPt = {}
cSin2PhiMinusPsiVsPt = {}
cCos4PhiMinusPsiVsPt = {}
vQ = {}
vQx = {}
vQy = {}
vV2ScalarProduct = {}
...
for det in detectors:
    ...
    for i_cent, cent in enumerate(centrality_classes):
        ...
        bin_df = complete_df.query(cent_seleration, inplace=False)
        for q in bin_df[f"fQ{det}"]:
            hQVsCent.Fill((cent[0] + cent[1]) / 2.0, q)
...
for i_cent, cent in enumerate(centrality_classes):
    ...
    for det in detectors:
        ...
        hCos2PhiMinusPsiVsPt = ROOT.TH1F(
            f"hCos{harmonic}PhiMinusPsiVsPt_{det}_cent_{cent[0]}_{cent[1]}",
            rf"; #it{{p}}_{{T}} (GeV/#it{{c}}); cos({harmonic}(#phi - #Psi_{{{det}}}))",
            n_pt_bins,
            pt_bins_arr,
        )
        ...
        hSin2PhiMinusPsiVsPt = ROOT.TH1F(
            f"hSin{harmonic}PhiMinusPsiVsPt_{det}_cent_{cent[0]}_{cent[1]}",
            rf"; #it{{p}}_{{T}} (GeV/#it{{c}}); sin({harmonic}(#phi - #Psi_{{{det}}}))",
            n_pt_bins,
            pt_bins_arr,
        )
        ...
        hCos4PhiMinusPsiVsPt = ROOT.TH1F(
            f"hCos4PhiMinusPsiVsPt_{det}_cent_{cent[0]}_{cent[1]}",
            rf"; #it{{p}}_{{T}} (GeV/#it{{c}}); cos(4(#phi - #Psi_{{{det}}}))",
            n_pt_bins,
            pt_bins_arr,
        )
        utils.setHistStyle(hCos4PhiMinusPsiVsPt, cent_colours[i_cent])
        vCos4PhiMinusPsiVsPt[det].append(hCos4PhiMinusPsiVsPt)
    ...
    for i_pt in range(0, n_pt_bins):
        ...
        for det in detectors:
            ...
            hCos2PhiMinusPsi_formatted_title = f";cos({harmonic}(#phi - #Psi_{{{det}}}); counts"
            hCos2PhiMinusPsi = ROOT.TH1F(
                f"hCos{harmonic}PhiMinusPsi_{det}_cent_{cent[0]}_{cent[1]}_pt_{i_pt}",
            ...
            hSin2PhiMinusPsi_formatted_title = f";sin({harmonic}(#phi - #Psi_{{{det}}}); counts"
            hSin2PhiMinusPsi = ROOT.TH1F(
                f"hSin{harmonic}PhiMinusPsi_{det}_cent_{cent[0]}_{cent[1]}_pt_{i_pt}",
            ...
            hCos4PhiMinusPsi_formatted_title = f";cos(4(#phi - #Psi_{{{det}}}); counts"
            hCos4PhiMinusPsi = ROOT.TH1F(
                f"hCos4PhiMinusPsi_{det}_cent_{cent[0]}_{cent[1]}_pt_{i_pt}",
                hCos4PhiMinusPsi_formatted_title,
                100,
                -1,
                1,
            )
            utils.setHistStyle(hCos4PhiMinusPsi, ROOT.kRed + 2)
            ...
            hV2ScalarProduct_formatted_title = (
                f";Q_{{{det}}} cos({harmonic}(#phi - #Psi_{{{det}}}); counts"
            )
            hV2ScalarProduct = ROOT.TH1F(
                f"hV{harmonic}ScalarProduct_{det}_cent_{cent[0]}_{cent[1]}_pt_{i_pt}",
                hV2ScalarProduct_formatted_title,
                200,
                -4,
                4,
            ...
            hV2ScalarProduct_check = ROOT.TH1F(
                f"hV{harmonic}ScalarProduct_check_{det}_cent_{cent[0]}_{cent[1]}_pt_{i_pt}",
                hV2ScalarProduct_check_formatted_title,
                200,
                -4,
                4,
            ...
            hV2Tree = ROOT.TH1F(
                f"hV{harmonic}Tree_{det}_cent_{cent[0]}_{cent[1]}_pt_{i_pt}",
                hV2ScalarProduct_formatted_title,
                200,
                -4,
                4,
            ...
            for phi, q, psi, v2 in zip(
                ...
                cos4PhiMinusPsi = ROOT.TMath.Cos(4 * delta_phi)
                ...
                hCos4PhiMinusPsi.Fill(cos4PhiMinusPsi)
                hQ.Fill(q)
                qx = q * ROOT.TMath.Cos(harmonic * psi)
                hQx.Fill(qx)
                qy = q * ROOT.TMath.Sin(harmonic * psi)
                hQy.Fill(qy)
                hV2ScalarProduct.Fill(q * cos2PhiMinusPsi)
                ux = ROOT.TMath.Cos(harmonic * phi)
                uy = ROOT.TMath.Sin(harmonic * phi)
            ...
            vCos4PhiMinusPsi[det][i_cent].append(hCos4PhiMinusPsi)
            ...
            vCos4PhiMinusPsiVsPt[det][i_cent].SetBinContent(
                i_pt + 1, hCos4PhiMinusPsi.GetMean()
            )
            vCos4PhiMinusPsiVsPt[det][i_cent].SetBinError(
                i_pt + 1, hCos4PhiMinusPsi.GetMeanError()
            )
    ...
    for det in detectors:
        ...
        vCos4PhiMinusPsiVsPt[det][i_cent].Write()

    vPhi.append(vPhi_cent)

for det in detectors:
    cSinPhiMinusPsiVsPt[det] = ROOT.TCanvas(
        f"cCos{harmonic}PhiMinusPsiVsPt_{det}", f"cCos{harmonic}PhiMinusPsiVsPt_{det}", 800, 600
    )
    ...
    cCos2PhiMinusPsiVsPt[det].DrawFrame(
        ...
        11.0,
        1.2,
        f";#it{{p}}_{{T}} (GeV/#it{{c}}); cos({harmonic}(#phi - #Psi_{{{det}}}))",
    )
    for i_cent, cent in enumerate(centrality_classes):
    ...
    cCos2PhiMinusPsiVsPt[det].SaveAs(
        f"{output_dir_name}/qc_plots/cCos{harmonic}PhiMinusPsiVsPt_{det}.pdf"
    )
    output_file.cd(f"{det}")
    cCos2PhiMinusPsiVsPt[det].Write()

    cSin2PhiMinusPsiVsPt[det] = ROOT.TCanvas(
        f"cSin{harmonic}PhiMinusPsiVsPt_{det}", f"cSin{harmonic}PhiMinusPsiVsPt_{det}", 800, 600
    )
    ...
    cSin2PhiMinusPsiVsPt[det].DrawFrame(
        ...
        11.0,
        1.2,
        f";#it{{p}}_{{T}} (GeV/#it{{c}}); sin({harmonic}(#phi - #Psi_{{{det}}}))",
    )
    for i_cent, cent in enumerate(centrality_classes):
    ...
    cSin2PhiMinusPsiVsPt[det].SaveAs(
        f"{output_dir_name}/qc_plots/cSin{harmonic}PhiMinusPsiVsPt_{det}.pdf"
    )
    output_file.cd(f"{det}")
    cSin2PhiMinusPsiVsPt[det].Write()

    cCos4PhiMinusPsiVsPt[det] = ROOT.TCanvas(
        f"cCos4PhiMinusPsiVsPt_{det}", f"cCos4PhiMinusPsiVsPt_{det}", 800, 600
    )
    cCos4Legend = ROOT.TLegend(0.40, 0.22, 0.70, 0.45, det, "brNDC")
    cCos4PhiMinusPsiVsPt[det].DrawFrame(
        0.0,
        -1.2,
        11.0,
        1.2,
        f";#it{{p}}_{{T}} (GeV/#it{{c}}); cos(4(#phi - #Psi_{{{det}}}))",
    )
    for i_cent, cent in enumerate(centrality_classes):
        vCos4PhiMinusPsiVsPt[det][i_cent].Draw("PE SAME")
        cCos4Legend.AddEntry(
            vCos4PhiMinusPsiVsPt[det][i_cent], f"{cent[0]}-{cent[1]}%", "PL"
        )
    cCos4Legend.Draw("SAME")
    cCos4PhiMinusPsiVsPt[det].SaveAs(
        f"{output_dir_name}/qc_plots/cCos4PhiMinusPsiVsPt_{det}.pdf"
    )
    output_file.cd(f"{det}")
    cCos4PhiMinusPsiVsPt[det].Write()
...
hResolution.SetTitle(r";FT0C percentile (%%); R_{%s}" % harmonic)
