...
for dir_name in input_dir_names:
    ...
    nuclei_hdl = TreeHandler(
        path, f'{nuclei_tree_name};', folder_name='DF*')
    ...
    # define new columns
    utils.redifineColumnsLight(nuclei_df)

    # apply mandatory selections
    nuclei_df.query(mandatory_selections + ' and ' + selections, inplace=True)

    hTPCdEdXvsP = ROOT.TH2D(f'hTPCdEdXvsP_{dir_name}', r';#it{p}/z (GeV/#it{c}); d#it{E}/d#it{X} (a. u.)', 25, 1., 6, 175, 0, 1400)

    hTPCdEdXvsP_toFit = ROOT.TH1D(f'hTPCdEdXvsP_toFit_{dir_name}', r';#it{p}/z (GeV/#it{c}); d#it{E}/d#it{X} (a. u.)', n_p_bins, p_bins_array)
    ...
    for i_p in range(0, n_p_bins):
        ...
        p_label = f'{p_bin[0]:.2f} ' + r'#leq #it{p}/z < ' + \
                f'{p_bin[1]:.2f}' + r' GeV/#it{c}'
    ...
    func_BB_default.SetParameters(
        default_parameters[0], default_parameters[1], default_parameters[2], default_parameters[3], default_parameters[4])
    ...
    cTPCdEdXvsP = ROOT.TCanvas(f'cTPCdEdXvsP_{dir_name}', f'cTPCdEdXvsP_{dir_name}', 800, 600)
    cTPCdEdXvsP.DrawFrame(0.8, 0., 4., 1400., r';#it{p}/z (GeV/#it{c}); d#it{E}/d#it{X} (a. u.)' )
